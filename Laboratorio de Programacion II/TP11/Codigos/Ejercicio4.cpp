...
public:
    ...
    double obtenerNotaMasAlta() const {
        return max(nota1, max(nota2, nota3));
    }

 
    double obtenerNotaMasBaja() const {
...
int main() {
    ...
    for (size_t i = 0; i < alumnos.size(); i++) {
        ...
        cout << "Nota más alta: " << alumnos[i].obtenerNotaMasAlta() << endl;
        cout << "Nota más baja: " << alumnos[i].obtenerNotaMasBaja() << endl;
        cout << endl;
    }
